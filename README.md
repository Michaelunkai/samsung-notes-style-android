...
- JSON backup export/import with schema/app/note-count metadata, expired-Trash import pruning, source-aware import status, deterministic imported-note merging, app-private rolling automatic backup snapshots with latest-snapshot restore and Settings status, plus per-note plain text export/share, rich HTML export, PDF export with page-style metadata and page-break pagination, and single-note Backup JSON export.
