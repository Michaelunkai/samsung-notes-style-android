...
The project is ready for the next controlled device gate only after the full local verification command passes for `0.5.30` and the debug APK exists. The device gate must follow `docs\DEVICE_TEST_PLAN.md` and must not start without explicit user approval.
